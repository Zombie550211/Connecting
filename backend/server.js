...
app.get('/api/graficas', protect, async (req, res) => {
  try {
    ...
    const [costumerResults, crmAgenteResults] = await Promise.all([
        Costumer.find({ fecha: fecha }).lean(),
        CrmAgente.find({ fecha: fecha }).lean()
    ]);

    // Combinar los resultados
    const resultados = [...costumerResults, ...crmAgenteResults];
